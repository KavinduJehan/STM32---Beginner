...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Main program body - Single Electronic Dice
  ******************************************************************************
  * @attention
  *
  * This example demonstrates a single electronic dice using 7-segment display
  * Target: STM32F103C8T6 (Blue Pill)
  * 7-segment display connected to GPIOB (PB0-PB6 for segments a-g)
  * Switch connected to PA0 (with internal pull-down)
  * 
  * Pin mapping for 7-segment display:
...
int main(void)
{
  ...
  while (1)
  {
    ...
    {
      // Switch just pressed - roll the dice
      uint8_t dice_value = Generate_Random_Number();
      
      // Show rolling animation
      for (int i = 0; i < 10; i++)
      {
        Display_Number(Generate_Random_Number());
        HAL_Delay(100);
      }
      
      // Display final result
      Display_Number(dice_value);
    }
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : PB0-PB6 (7-segment display) */
